/**
 * Detekce změn v obrazu (detekce "pohybu v obraze") přímo v ESP32-CAM,
 * bez PIR čidla.
 * 
 * Fotky posílá do galerie na serveru https://ratatoskr.lovecka.info/
 * 
 * Fotky posílá 
 * - při změně v obrazu
 * - když se nic neděje, tak jednou za 20 minut (SEND_PICTURE_PERIODICALLY_INTERVAL_MS)
 */


// konfigurace pro build:
...
// konfigurace detekce pohybu je níže na konci sekce s includy a definicemi objektů
...
// detekce změn v obrazu
#include "MotionDetector.h"
MotionDetector motionDetector( &serialLogger, &appState );


// ----- konfigurace detekce změn v obraze -------

// Chceme po detekci pohybu poslat jeste jednu fotku o chvili pozdeji? (0/1)
#define SEND_AFTER_MOTION 1
// po jake dobe se posle?
#define AFTER_MOTION_DELAY_MS 500
// kolik ms po detekci pohybu je další pohyb ignorován
#define MOTION_AFTER_MOTION_NUMBNESS 15000

/* 
Pohyb se detekuje na fotce 640x480, o cca 250 msec později se vyfotí fotka v plném rozlišení.
Má se do galerie poslat i ta úvodní malá fotka? (0/1) */
#define SEND_LEADER_PHOTO 1

/* 
Konfigurace detekce změn obrazu.

Detektor si rozebere obrázek 640x480 na bloky 16x8 px, tedy 40x60 bloků (celkem 2400 bloků).
Pro každý blok spočte průměrnou hodnotu barevných složek R,G,B.
Porovnává celkovou odchylku v konkrétním bloku mezi jednotlivými snímky.
*/
// kolik bloků se musí minimálně změnit, aby se detekoval pohyb?
#define MOTION_MINIMUM_CHANGE_BLOCKS 50
// o kolik se musí bloky minimálně lišit, aby byly brány jako rozdílné (součet absolutní odchylky ve třech barevných kanálech)
#define MOTION_MINIMUM_CHANGE_VALUE 10

// OV2640: tato kamera ma vetsi sum, minimalni zmena musi byt vetsi
// kolik bloků se musí minimálně změnit, aby se detekoval pohyb?
#define MOTION_MINIMUM_CHANGE_BLOCKS_OV2640 50
// o kolik se musí bloky minimálně lišit, aby byly brány jako rozdílné (součet absolutní odchylky ve třech barevných kanálech)
#define MOTION_MINIMUM_CHANGE_VALUE_OV2640 30

// nahrát periodicky fotku, i když není detekován pohyb? (0/1)
#define SEND_PICTURE_PERIODICALLY 1
// a jak často? (msec)
#define SEND_PICTURE_PERIODICALLY_INTERVAL_MS 1200000
// prvni fotka se posle N ms po startu
#define SEND_PICTURE_FIRST_PHOTO_MS 30000


// motion detector pojedeme na malem rozliseni, aby se to delalo dost rychle 
// (2048x1536 sezere 1900 msec na jeden snimek, VGA uz jen 200)
// nazvy formatu: https://github.com/espressif/esp32-camera/blob/master/driver/include/sensor.h
#define ROZLISENI_PRO_MOTION_DETECTOR FRAMESIZE_VGA
#define ROZLISENI_PRO_MOTION_DETECTOR_X 640
#define ROZLISENI_PRO_MOTION_DETECTOR_Y 480
...
void setup() {
  ...
  // rozliseni pro motion detector
  cameraHelper.setResolution( ROZLISENI_PRO_MOTION_DETECTOR );
  // konfigurace detektoru
  motionDetector.init( ROZLISENI_PRO_MOTION_DETECTOR_X, ROZLISENI_PRO_MOTION_DETECTOR_Y );
  int defaultMinChangeBlocks = MOTION_MINIMUM_CHANGE_BLOCKS;
  int defaultMinChangeValue = MOTION_MINIMUM_CHANGE_VALUE;
  if( TYP_KAMERY == CAMERA_OV2640 ) {
    // pro tuto kameru s vyssim sumem dame vyssi limity
    defaultMinChangeBlocks = MOTION_MINIMUM_CHANGE_BLOCKS_OV2640;
    defaultMinChangeValue = MOTION_MINIMUM_CHANGE_VALUE_OV2640;
  }
  motionDetector.setMinimalBlocksChanged( config.getLong("camera_md_minblocks", defaultMinChangeBlocks ) );
  motionDetector.setMinimalBlockValueDifference( config.getLong("camera_md_minchange", defaultMinChangeValue ) );
  motionDetector.setAfterMotionNumbnessMs( MOTION_AFTER_MOTION_NUMBNESS );
  ...
  tasker.setTimeout( captureAndAnalyzeMotion, 500 );

  if( SEND_PICTURE_PERIODICALLY ) {
    tasker.setTimeout( captureAndSendPeriodically, config.getLong( "camera_first_take", SEND_PICTURE_FIRST_PHOTO_MS ) );
  }
...
void userRoutes( AsyncWebServer * server )
{
  ...
  server->on("/snap", HTTP_GET, onRequestSnap );
}
...
/** 
 * Voláno z taskeru. Pořídí fotku v malém rozlišení, analyzuje ji,
 * a pokud došlo k pohybu, pořídí ještě v plném rozlišení a obě pošle.
 */
void captureAndAnalyzeMotion() {
    ...
    if( ! cameraHelper.hasImage() ) {
      serialLogger.log( "nemam obrazek" );
      tasker.setTimeout( captureAndAnalyzeMotion, 100 );
      return;
    }

    int res = motionDetector.analyze( cameraHelper.img.data, cameraHelper.img.size, cameraHelper.img.w, cameraHelper.img.h );

    if( res==2 ) {
      // detekce pohybu
      serialLogger.log("detekovan pohyb!" );

      // ulozime si current obrazek do savebufferu
      cameraHelper.saveImage();

      // nafotime full size
      cameraHelper.setMaximalResolution();
      cameraHelper.capture();
      cameraHelper.setResolution( ROZLISENI_PRO_MOTION_DETECTOR );

      // az ted posleme oba obrazky 
      if( SEND_LEADER_PHOTO ) {
        sendImage( 2, &(cameraHelper.savedImg) );
      }
      sendImage( 1, &(cameraHelper.img) );

      if( SEND_AFTER_MOTION ) {
        // posleme jeste jednu fotku o chvili pozdeji?
        // vyssi priorita
        tasker.setTimeout( captureFullSize, AFTER_MOTION_DELAY_MS, 3, (byte)0 );    
      }
      
      if( SEND_PICTURE_PERIODICALLY ) {
        // teď jsme poslali fotku s pohybem, takže odložíme fotku posílanou v případě, že se nic nehýbe, zase o celý interval
        tasker.setTimeout( captureAndSendPeriodically, config.getLong( "camera_send_every", SEND_PICTURE_PERIODICALLY_INTERVAL_MS ) );  
      }
    }    

    // po 100 ms vezmeme dalsi fotku
    // nastavíme nejnižší prioritu
    tasker.setTimeout( captureAndAnalyzeMotion, 100, (byte)25  );    
}


/**
 * Udělá jednu fotku v plném rozlišení a pošle jí na server.
 * 
 * mode = 3 pro after-motion fotku, 0 pro běžnou periodickou fotku
 */
void captureFullSize( int mode ) {
    cameraHelper.setMaximalResolution();
    cameraHelper.capture();
    cameraHelper.setResolution( FRAMESIZE_VGA );
    sendImage( mode, &(cameraHelper.img) );
}


/** volano z taskeru  */
void captureAndSendPeriodically()
{
  captureFullSize( 0 );
  // aby přepínání rozlišení na OV2640 nevytriggerovalo hned detekci pohybu
  motionDetector.imgSent();
  tasker.setTimeout( captureAndSendPeriodically, config.getLong( "camera_send_every", SEND_PICTURE_PERIODICALLY_INTERVAL_MS ) );  
  tasker.setTimeout( captureAndAnalyzeMotion, 100, (byte)25  );    
}
...
/**
 * Pošle aktuální fotku na server.
 * Voláno z captureAndAnalyzeMotion(), pokud je detekován pohyb.
 * Voláno z captureAndSendPeriodically() resp. captureFullSize pro poslání periodické fotky.
 */
void sendImage( int mode, SavedImage * img ) {
  ...
  serialLogger.log( "odesilam: %dx%d px, %d b, #%d", 
      img->w, img->h, 
      img->size, cameraHelper.imagesTaken );

  if( mode==1 ) {
    strcpy( name, "camera%20motion" );
  } else if( mode==2 ) {
    sprintf( name, "camera%%20motion-leader%%20blk:%d%%20l:%d", motionDetector.numBlocksChanged, motionDetector.blockChangeLimit );
  } else if( mode==3 ) {
    strcpy( name, "camera%20after-motion" );
  } else {
    if( motionDetector.blackPhoto ) {
      strcpy( name, "camera%20timer%20black" );
    } else {
      strcpy( name,  "camera%20timer" );
    }
    
  }

  if( imageSender.sendImage( img->data, img->size, name ) ) {
    imagesSent++;
  } else {
    imagesNotSent++;
  }
...
void vlozInformace( AsyncResponseStream *response )  {
  ...
  if( motionDetector.lastMotionDetected!=0 ) {
    response->printf( "<br>Poslední pohyb před %d sec.",
      (millis() - motionDetector.lastMotionDetected) / 1000L
    );
  }
...
void onRequestRoot(AsyncWebServerRequest *request){
  ...
  response->print("<p><a href=\"/snap\">Pošli fotku do galerie!</a></p>" );
...
void onRequestSnap(AsyncWebServerRequest *request) {
  asyncLogger.log( "@ req snap" );

  tasker.setTimeout( captureAndSendPeriodically, 1 );  
...
// ESP32-S3-CAM (se dvěma USB-C porty)
